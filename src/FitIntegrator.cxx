...
FitIntegrator::FitIntegrator(std::shared_ptr<const FitModel> fit_model,
    ...
    Data_(FitModel_->model()->createDataSet()), // there's no data yet
    Partitions_(NumberOfThreads_, &Data_),
    ModelIntegral_(*fit_model->model()),
    PointGenerator_([=]() { return yap::phsp(*FitModel_->model(), FitModel::Dmass(), FitModel_->massAxes(), FitModel_->massRanges(), RandomEngine_, std::numeric_limits<unsigned>::max()); })
{
